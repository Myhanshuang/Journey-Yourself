...
export default function TimelineView() {
  ...
  const [daySelection, setDaySelection] = useState<EntryCard[] | null>(null)
  ...
  const { data: diaries = [], isLoading } = useQuery({
    queryKey: ['app', 'timeline', filterId],
    queryFn: () => appQueryApi.timelineAll({ notebookId: filterId, limit: 100 })
  })

  // ✅ 修复 1：用 useMemo 缓存分组数据
  const groupedData = useMemo(() => {
    return diaries.reduce<Record<string, Record<string, EntryCard[]>>>((acc, d) => {
  ...
  const timelineItems = useMemo(() => {
    ...
    years.forEach(year => {
      ...
      Object.keys(groupedData[year])
        ...
        .forEach(month => {
          groupedData[year][month].forEach((diary) => {
    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="py-12 space-y-10 min-h-screen relative text-slate-900 max-w-[840px] mx-auto">
      ...
      <header className="flex flex-col gap-6 md:gap-10">
        ...
        <div className="flex items-center gap-2 md:gap-3 overflow-x-auto no-scrollbar pb-2 text-slate-900">
          ...
          {notebooks.map((nb) => <FilterButton key={nb.id} label={nb.name} active={filterId === nb.id} onClick={() => setFilterId(nb.id)} />)}
        </div>
      </header>
        ...
        {isLoading ? <div className="py-40 text-center animate-pulse text-slate-300 font-black uppercase tracking-widest text-sm">Synchronizing...</div> :
          mode === 'stream' ? (
            <motion.div key="stream" className="space-y-32 relative pt-4">
              {years.map(year => (
                <div key={year} className="space-y-16 relative" id={`diary-section-year-${year}`}>
                    ...
                    return (
                      <div key={key} className="space-y-8 relative">
                        ...
                        <div className="grid grid-cols-1 gap-6 relative z-10 px-4">
                          {groupedData[year][month].map((diary) => (
            ...
            <CalendarGrid diaries={diaries} onDateClick={(ds: EntryCard[]) => ds.length === 1 ? handleDiaryClick(ds[0]) : setDaySelection(ds)} />
          )}
      </AnimatePresence>
    </motion.div>
  )
