...
import { useAdjustedTime, useConfirm, useToast, useIsMobile, cn, ActionMenu, getBaseUrl } from '../../components/ui/JourneyUI'
...
import { appQueryApi, type EntryDetail } from '../../shared/api/appQuery'
...
export default function DiaryDetailView() {
  ...
  // Fetch diary by ID
  const { data: diary, isLoading, error } = useQuery<EntryDetail>({
    queryKey: ['app', 'entry', Number(id)],
    queryFn: () => appQueryApi.entryDetail(Number(id)),
    enabled: !!id,
  })
  ...
  useLayoutEffect(() => {
    let frameId = 0
    const markReady = () => {
      frameId = window.requestAnimationFrame(() => setContentReady(true))
    }
    ...
    askConfirm(
      ...
      async () => {
        try {
          ...
          back()
        } catch { addToast('error', 'Failed to erase memory') }
      }
    )
  }
