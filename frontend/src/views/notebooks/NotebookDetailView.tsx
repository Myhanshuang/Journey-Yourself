...
import { appQueryApi, type EntryCard, type NotebookDetail } from '../../shared/api/appQuery'

interface OutletContextType {
  notebooks: Array<{ id: number; name: string }>
  setNotebookModal: (config: { show: boolean; data?: NotebookDetail | undefined; afterCreate?: () => void }) => void
...
export default function NotebookDetailView() {
  ...
  const { data: notebook, isLoading: isLoadingNotebook } = useQuery<NotebookDetail>({
    queryKey: ['app', 'notebook', Number(id), 'detail'],
    queryFn: () => appQueryApi.notebookDetail(Number(id)),
    enabled: !!id,
  })

  // Fetch diaries with pagination
  const { data: diaries = [] } = useQuery<EntryCard[]>({
    queryKey: ['app', 'notebook', Number(id), 'entries'],
    queryFn: () => appQueryApi.notebookEntriesAll(Number(id), { limit: 100 }),
    enabled: !!id,
  })
